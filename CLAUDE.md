...
- **macOS:** GPU/VRAM and CPU temperature/power are still not implemented in v1.3 — those panels show N/A. NVML is NVIDIA-only, and Apple Silicon SMC sensor coverage in `sysinfo` is not yet sufficient. CPU, memory, network, disk, processes, and process kill all work.
